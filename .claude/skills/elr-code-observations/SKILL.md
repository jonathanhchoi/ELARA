...
4. Use the Claude workflow adapter specified by the shared contract. If its required mode is not
   active, issue the exact handoff for `/elr-code-observations` instead of imitating that mode.
5. Validate returns and update ledgers serially after each bounded wave. Resume from files,
