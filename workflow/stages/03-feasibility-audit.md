...
6. **Gate 4: Can we obtain and use the data the project needs?** Verify with live probes that the corpus exists, its authoritative source and access route, dates and jurisdictions covered, document formats, metadata fields, selection layers, known gaps, deduplication identifiers, OCR quality, API behavior, rate limits, and current terms or license status. Pull a small, authorized sample and confirm it contains the information proposed for coding. Preserve raw probe responses and typed failures. Reuse the Stage 02 smoke-screen probes rather than repeating them. Record every unit whose content is inspected or coded during any probe in probe_exposure_manifest.csv — unit or document identifier, hash where available, variables inspected or coded, and probe ID — so Stage 04 partitions and the Stage 13 held-out validation frame can quarantine those units.
7. **Gate 5: Will there be enough usable data to answer the research question?** Estimate incidence from authorized marker counts or a transparently coded mini-sample, never intuition, and add every mini-sample unit to the probe-exposure manifest. Project the funnel from the raw universe through availability, eligibility, readability, coding success, and analysis inclusion. Give low, central, and high scenarios and identify assumptions. Then compute the minimum detectable effect for each planned primary comparison at a researcher-approved power and significance level — present 0.80 and 0.05 as defaults for explicit approval — under each funnel scenario, inflated for anticipated coding errors and for clustering or design effects. Archive the formula or script and its assumptions so Stage 04 and the preregistration can quote them. If the minimum detectable effect exceeds every effect size plausible against the closest literature, this gate fails: the design cannot answer its question, and the verdict should say so in plain language rather than deferring the discovery to analysis.
...
9. **Gate 6: Can the project be completed in a reasonable amount of time with the available resources?** Use ELARA's host-orchestrated sub-agent harness as the default route for later coding. Do not assign a dollar value to subscription-backed sub-agent use. For low, central, and high scenarios, estimate wall-clock completion time from the expected assignments, bounded concurrent worker capacity recorded in the active access snapshot, observed or probed per-assignment duration, retries, serial validation, and aggregation overhead. If an authorized timed probe is unavailable, expose the unverified duration assumption and use a range. Express human validation and other resource burdens primarily as time or capacity requirements; record known fixed charges and flag potential spending decisions, but do not invent dollar estimates. Separately include a counterfactual API-price comparison in every audit. Label the API as an optional execution alternative rather than the default and explain that its per-token price is not equivalent to subscription-backed sub-agent use. Calculate the comparison from projected documents and tokens at each funnel stage, current provider prices, retries, plausible model tiers, and available batch discounts. Save formulas and price-sheet URLs with access dates; do not recall prices. When any candidate data route is licensed, restricted, confidential, or human-subjects, also estimate the authorization lead time — license negotiation, data-use agreements, IRB or ethics review — from current published evidence, include low, central, and high lead-time scenarios in the schedule range, and treat a lead time incompatible with the researcher's stated timeline as a binding constraint.
...
11. **Gate 8: Does a legal, ethical, data-use, or spending issue require the researcher’s decision?** Identify every issue touching terms of service, licenses, institutional rules, restricted or human-subjects data, confidentiality, or nontrivial spending. Flag each issue for the researcher and do not resolve it or treat silence as approval.
12. Write a gate-by-gate table using the eight exact question headings above and pass, pass with conditions, or fail, with evidence IDs. Name one binding constraint, the probe result, the funnel, the minimum detectable effects, sub-agent completion-time scenarios, counterfactual API-price range, risks, unverified assumptions, and a verdict of go, go with modifications, or no-go. State exactly what evidence would change the verdict and report no confidence score.

## Artifacts

feasibility_audit_vNNN.md is the decision document. probe_log.csv must record probe ID, hypothesis, method, URL or local source, access time, authorization basis, exact result, raw artifact path, and interpretation. probe_exposure_manifest.csv lists every unit whose content was inspected or coded during the audit, so later stages can quarantine those units from held-out validation. funnel_model.csv and cost_model.csv must expose assumptions and formulas — including the minimum-detectable-effect and validation-precision calculations — rather than only totals. Retain the cost_model.csv name for compatibility. It must distinguish the default sub-agent route from the optional API alternative and record the scenario, expected assignments, concurrency, observed or assumed duration, retries, serial overhead, projected wall-clock time, and, for the API comparison only, token and current-price calculations. Mark the sub-agent dollar cost as not estimated, not zero. Record human and other resource requirements as time or capacity, plus known fixed charges where they exist. variable_verifiability.csv must cover every proposed variable. Preserve raw requests, responses, samples, screenshots where necessary, and typed gap records under probes/.
...
- Recalculate the low, central, and high sub-agent completion times from expected assignments, bounded concurrency, observed or probed duration, retries, serial validation, and aggregation overhead.
- Confirm the audit does not assign a dollar value to subscription-backed sub-agent use and includes a separately labeled, counterfactual API-price comparison from recorded current prices, token assumptions, retries, model tiers, and available batch discounts.
