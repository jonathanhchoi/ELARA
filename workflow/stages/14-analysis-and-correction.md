...
4. Encode the preregistered inclusion, exclusion, aggregation, weighting, model, missing-data, multiple-comparisons correction, and per-estimand clustering rules literally. Report unadjusted and family-adjusted inference side by side for every confirmatory estimate, per the preregistered correction. Label authorized post-registration work as exploratory in code, filenames, tables, and the report. Append any material departure before running affected results.
