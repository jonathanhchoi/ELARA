...
10. Enumerate unit_space_vNNN.csv from the approved frame. Give each row a stable unit ID and record source identifier, expected document or component, inclusion status and rule, partition eligibility, source location, metadata provenance, and typed availability status. Include excluded and missing candidates where needed to reconcile the frame. Record the construction query, snapshot date, row count, and SHA-256 hash. Before recording the hash, audit closure against the source, not only internally: confirm no two rows are the same underlying unit (duplicates, superseded versions, or separately indexed components of one unit); derive classification fields such as court or jurisdiction from the source's authoritative registry, never from identifier patterns; cover every alternate record or identifier the source assigns to the same anchor or unit; and reconcile the enumerated count against the source's own reported totals for each construction query. Record the reconciliation and each discrepancy's resolution under the run directory; an unexplained shortfall is an enumeration failure, not a rounding note.
