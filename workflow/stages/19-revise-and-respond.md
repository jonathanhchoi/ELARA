...
2. Parse each discrete reviewer, editor, researcher, and citation-audit issue into a stable comment ID. Keep compound comments together only when they require one inseparable decision.
...
3. For an accepted prose or citation issue, make the narrowest change that fully responds. Preserve the researcher's voice and unchanged text. Retrieve and read the actual authority before adding or replacing a citation; never fabricate a source, quotation, pinpoint, or characterization.
...
9. Compile or render the complete manuscript and inspect it visually and structurally. Conduct a full consistency pass across claims, numbers, citations, tables, figures, cross-references, disclosures, and appendices, followed by a diff review for out-of-scope edits.
10. Draft a respectful, cooperative response letter organized by comment ID that quotes or fairly paraphrases each request, states the disposition, describes the precise change and location, reports new results accurately, and explains any principled decline without defensiveness.
