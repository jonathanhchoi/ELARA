...
13. Instantiate `workflow/templates/preemption_review_template.md` as the run-scoped `project/runs/<run_id>/preemption_review_source.md`. Preserve the required section order and complete every metadata field. Begin with a decision-focused executive summary centered on direct comparisons between the intended contribution and every closest match that materially affects the verdict. State the intended contribution exactly. For each closest match, state what the work actually says, its relevant scope and evidentiary or methodological basis, the precise component of the intended contribution it occupies, what remains distinguishable, and the source-manifest and claim-evidence IDs or pinpoint pages supporting that comparison. Then synthesize the remaining contribution, recommended disposition, scoop risk, material access gaps, and recheck date. Do not treat topical similarity alone as preemption, group materially distinct works into one comparison, or introduce a claim or source absent from the verified detailed review. Keep the summary within roughly two or three rendered pages and the builder's 1,200-word limit. Unless the researcher expressly selected another format, generate `project/artifacts/preemption_review_vNNN.tex` with `python scripts/build_preemption_review.py project/runs/<run_id>/preemption_review_source.md project/artifacts/preemption_review_vNNN.tex`, compile it to `project/artifacts/preemption_review_vNNN.pdf`, and archive the build log. If the researcher selected Word, pass a `.docx` output path to the same builder and record that preference. The Markdown file is an auditable build source, not the active researcher-facing report. Render the complete PDF or expressly selected alternative under `project/runs/<run_id>/rendered_preemption_review/`, inspect every page at 100 percent zoom, and iterate on the source until headings, paragraphs, lists, hyperlinks, page breaks, headers, and footers are clean. Never patch generated `.tex`, PDF, or DOCX output by hand. If the required compiler or renderer is unavailable, record the gap, leave the artifact inactive, and set `waiting_for_user` with the exact capability needed.

## Artifacts

preemption_review_vNNN.pdf is the default active, researcher-facing literature review, compiled from the versioned LaTeX source. An expressly requested alternative such as DOCX may be active instead, with the preference recorded. The report must contain, in order, the concise executive summary, annotated map, verdict and flip conditions, positioning and lineage, scoop risk, search methods and saturation evidence, access limitations and manual search packet, and review date. The annotated map begins on a new page after the executive summary. The run-scoped preemption_review_source.md is the exact build source and remains immutable when the run closes. The source manifest, query-level search log, claim-evidence table, retrieved copies, rendered page images, and run manifest are mandatory support artifacts. Do not cite a source absent from the manifest.
