...
declared_outputs: ["project/artifacts/preemption_review_vNNN.md", "project/sources/preemption/<run_id>/source_manifest.csv", "project/sources/preemption/<run_id>/search_log.csv", "project/sources/preemption/<run_id>/claim_evidence.csv", "project/sources/preemption/<run_id>/retrieved/", "project/sources/preemption/<run_id>/fanout/ (query matrix, worker briefs and prompts, fan-out manifest, worker returns, merged candidates)", "project/runs/<run_id>/run_manifest.json", "project/PROJECT_STATE.md", "project/DECISIONS.md", "project/RUN_LEDGER.md", "project/DEVIATIONS.md"]
...
4. Run at least fifteen genuinely distinct queries, broad before narrow, across at least four suitable routes: scholarly search or OpenAlex; law-school repositories and law-review sites; SSRN and working-paper title searches; relevant disciplinary indexes or repositories such as NBER, OSF, arXiv, SocArXiv, or institutional archives; and the open web for conference programs, symposia, calls, datasets, dissertations, and works in progress. Use available subscription databases only within the researcher's authorization. When the queries, author searches, citation chains, retrievals, or the fresh review are parallelized, follow the "Worker tool surface, time boxes, and crash-resume" section of `workflow/shared/observation-fanout.md`: one query, author, chain, or work per `elr-research-worker` (never an all-tools default agent), briefs, prompts, a fan-out manifest, and every worker return under `project/sources/preemption/<run_id>/fanout/`, at most six workers at once with a parent watchdog and bounded attempts, and a ledger checkpoint after each wave. Sites behind bot walls or paywalls (SSRN's own pages, HeinOnline, Westlaw, Lexis, JSTOR, Google Scholar) are reached only through open APIs and indexes or the researcher's own authenticated session; a worker that meets a 401/403/429, CAPTCHA, or login wall records a typed access gap and moves on, and those gaps feed the manual search packet in step 11.
